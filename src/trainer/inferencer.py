...
class Inferencer:
    """
    Template-standard inferencer.

    Usage (from train.py after training):
        inferencer = Inferencer(config, dataloaders["test"], model, device, logger)
        inferencer.predict()

    Usage (from infer.py as standalone):
        model = instantiate(config.model).to(device)
        _load_checkpoint(model, config, device, logger)   # done in infer.py
        inferencer = Inferencer(config, dataloaders["test"], model, device, logger)
        inferencer.predict()
    """
    def __init__(
        ...
        self.plot_dir = (
            ...
            or getattr(trainer_cfg, "plot_dir", trainer_cfg.save_dir)
        )
        self.class_names = (
            list(getattr(infer_cfg, "class_names", None) or ["A", "B", "C", "D", "E"])
        )
    ...
    def predict(self) -> None:
        ...
        self.logger.info(
            "Macro  Precision=%.2f%% Recall=%.2f%% F1=%.2f%%",
            macro_p * 100, macro_r * 100, macro_f1 * 100,
        )
        self.logger.info(
            "Micro  Precision=%.2f%% Recall=%.2f%% F1=%.2f%%",
            micro_p * 100, micro_r * 100, micro_f1 * 100,
        )
        self.logger.info(
            "Weighted Precision=%.2f%% Recall=%.2f%% F1=%.2f%%",
